...
class GetTokenTests(unittest.TestCase):
    ...
    def test_session_mode_never_requests_magic_link_refresh(self) -> None:
        user_data_dir, temp_profile = get_token.prepare_user_data_dir("session", None)
        ...
        class _Page:
            ...
            def get_by_role(self, *args, **kwargs):
                raise AssertionError("session mode should not request magic-link UI")
            ...
            with mock.patch.object(get_token, "sync_playwright", return_value=_PlaywrightCM(playwright)):
                with mock.patch.object(get_token, "_reexec_into_integrations_venv_if_needed", return_value=None):
                    with mock.patch.object(get_token, "_reexec_under_xvfb_if_needed", return_value=None):
                        with mock.patch.object(get_token, "wait_for_magic_link", side_effect=AssertionError("magic link fallback should not run")):
                            with mock.patch.object(get_token.sys, "argv", argv):
                                with mock.patch.object(get_token, "token_is_fresh", return_value=False):
                                    rc = get_token.main()

        self.assertEqual(rc, 1)
        self.assertEqual(page.url, "https://app.copilot.money/transactions")

    def test_session_mode_drives_transactions_route_to_capture_token(self) -> None:
        ...
        class _Page:
            ...
            def evaluate(self, script):
                if self.url == "https://app.copilot.money/transactions":
